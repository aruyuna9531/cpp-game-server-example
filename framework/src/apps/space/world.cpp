...
{
    LOG_DEBUG("World::HandleNetworkDisconnect world id:" << _worldId << " disconnect." << pPacket);
...
void World::BroadcastPacket(Proto::MsgId msgId, google::protobuf::Message& proto)
{
    ...
    {
        LOG_DEBUG("broadcast all, msgId:" << Log4Help::GetMsgIdName(msgId).c_str() << " player sn:" << pair.second->GetPlayerSN());
...
void World::BroadcastPacket(Proto::MsgId msgId, google::protobuf::Message& proto, std::set<uint64> players)
{
    ...
    for (const auto one : players)
    {
        ...
        LOG_DEBUG("broadcast parts, msgId:" << Log4Help::GetMsgIdName(msgId).c_str() << " player sn:" << one);
...
void World::HandleG2SRemovePlayer(Player* pPlayer, Packet* pPacket)
{
    ...
    LOG_DEBUG("World::HandleG2SRemovePlayer remove player " << proto.player_sn());
...
void World::HandleMove(Player* pPlayer, Packet* pPacket)
{
    ...
    for (auto index = 0; index < proto.position_size(); index++)
    {
        ...
        LOG_DEBUG("World::HandleMove: sn " << pPlayer->GetPlayerSN() << " move target " << v3);
    }
