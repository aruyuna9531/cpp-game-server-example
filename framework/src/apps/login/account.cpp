...
void Account::HandleAccountCheck(Packet* pPacket)
{
    ...
    LOG_DEBUG("account check account:" << protoCheck.account() << pPacket);
    ...
    LOG_DEBUG("account " << protoCheck.account().c_str() << " is added to player controller.");
...
void Account::HandleAccountQueryOnlineToRedisRs(Packet* pPacket)
{
    ...
    if (pPlayer == nullptr) {
        LOG_ERROR("Account::HandleAccountQueryOnlineToRedisRs failed, pPlayer = null");
        return;
    }
    ...
    // 验证账号，发起一个Http请求
    LOG_DEBUG("Creating connect to player at " << _httpIp << ":" << _httpPort << "...");
...
void Account::HandleHttpOuterResponse(Packet* pPacket)
{
    ...
    {
        LOG_DEBUG("OK, send client S2C_EnterWorld! player: sn " << pPlayer->GetPlayerSN() << ", name " << pPlayer->GetName() << ", account " << pPlayer->GetAccount());
