...
public:
    ...
    AABB3(const Vector3& _min, const Vector3& _max) {
        min = _min;
        max = _max;
    }
    Vector3 size() const;
    ...
    bool contains(const Vector3& p) const;  // 包含点p返回true;
    bool contains(const AABB3& aabb) const;  // 包含aabb返回true
...
inline std::ostream& operator<<(std::ostream& os, const AABB3& aabb) {
