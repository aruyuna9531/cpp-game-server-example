...
void ThreadMgr::UpdateDispatchPacket()
{
    ...
    for (auto iter = pList->begin(); iter != pList->end(); ++iter)
    {
        ...
        if (pPacket->GetSocketKey()->Socket != INVALID_SOCKET) {
            LOG_DEBUG("Receive packet, message id " << Proto::MsgId_Name(pPacket->GetMsgId()) << ", socket " << pPacket->GetSocketKey()->Socket << ", network type " << GetNetworkTypeName(pPacket->GetSocketKey()->NetType))
        }
