...
{
    LOG_DEBUG("Send packet, message id " << Proto::MsgId_Name(msgId) << ", app type " << GetAppName(appType) << ", appId" << appId);
...
{
    LOG_DEBUG("Send packet, app type " << GetAppName(appType) << ", appId" << appId);
    if ((Global::GetInstance()->GetCurAppType() & appType) != 0)
    {
        DispatchPacket(pPacket);
        return;
...
{
    LOG_DEBUG("Send packet, message id " << Proto::MsgId_Name(msgId) << ", app type " << GetAppName(appType) << ", appId" << appId << ", tag key exist, with proto");
    if ((Global::GetInstance()->GetCurAppType() & appType) != 0)
    {
...
{
    LOG_DEBUG("Send packet, message id " << Proto::MsgId_Name(msgId) << ", app type " << GetAppName(appType) << ", appId" << appId << ", tag key exist");
    if ((Global::GetInstance()->GetCurAppType() & appType) != 0)
    {
        NetIdentify identify;
        identify.GetTagKey()->CopyFrom(pTagKey);
